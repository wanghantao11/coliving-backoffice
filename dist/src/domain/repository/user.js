...
let UserRepository = class UserRepository {
    constructor() {
        this.REPO_NAME = 'user';
        this.deleteUserByExternalId = (iduser) => typeorm_1.getRepository(entity_1.User, process.env.NODE_ENV).createQueryBuilder('user')
            .delete()
            .from(entity_1.User)
            ...
            .execute()
            .then(res => res.raw[0]);
        this.createUser = (user) => Promise.resolve(Object.assign(user, { email: common_1.formatEmail(user.email) }))
            .then(user => typeorm_1.getRepository(entity_1.User, process.env.NODE_ENV)
            ...
            .execute()
            .then(res => res.raw[0]));
        this.getUserBy = (filter) => {
            const { email, iduser, is_verified = true } = filter;
            const queries = [];
            const initBuilder = typeorm_1.getRepository(entity_1.User, process.env.NODE_ENV)
                .createQueryBuilder(this.REPO_NAME)
                .select([
                '\"user\".first_name',
                '\"user\".last_name',
        ...
        this.updateUserBy = (filter, data) => {
            ...
            return typeorm_1.getRepository(entity_1.User, process.env.NODE_ENV)
                ...
                .update()
                .set(Object.assign({}, common_1.excludeKeysFromObject('age')(data)))
                .where(queries.join(' AND '))
                .returning([
                'first_name',
                'last_name',
        ...
        this.getUsersCountBy = (filter = {}) => {
            ...
            if (ageSpan) {
                queries.push(`birthday > '${common_1.convertDateToISOString(ageSpan.minBirthday)}' AND birthday <= '${common_1.convertDateToISOString(ageSpan.maxBirthday)}'`);
            }
            if (locations) {
        ...
        this.getUsersBy = (filter) => {
            const queries = [];
            const { ageSpan, registrationTimeSpan, locations, offset = 0, limit = 10, exclude = [], idusers = [], userKeys = [], user_type, is_verified = true } = filter;
            const initBuilder = typeorm_1.getRepository(entity_1.User, process.env.NODE_ENV)
                .createQueryBuilder(this.REPO_NAME)
                .leftJoinAndSelect('user.preferences', 'preferences')
                .select([
                ...
                .offset(offset)
                .limit(Math.min(limit, constants_1.MAX_USER_LIMIT));
            if (exclude.length !== 0) {
                queries.push(`user.iduser != ANY('{${exclude.join(',')}}')`);
            }
            if (registrationTimeSpan) {
        ...
        this.getSubscribedMembers = (facadeId, filter) => {
            const { email, name, has_single_room, has_double_room, has_private_bathroom, has_shared_bathroom, has_private_toilet, has_shared_toilet, include_unspecified_age, include_unspecified_move_in_date, exclude_users_with_offers, exclude_current_tenants, is_only_matching_rent, is_only_suitable_for_disability, is_only_today, is_only_test_complete, rent, tag_ids, age_from = 18, age_to = 100, subscribe_from, subscribe_to, move_in_date_from, offset = 0, limit = 10, sort_by = 'subscribed_at', sort_order = 'DESC', } = filter;
            ...
            if (common_1.convertStringToBoolean(is_only_test_complete)) {
            ...
            if (common_1.convertStringToBoolean(exclude_current_tenants)) {
                filterQuery = filterQuery + 'AND user.user_type != \'Tenant\' ';
            }
            if (common_1.convertStringToBoolean(exclude_users_with_offers)) {
         ...
         AND user.birthday > '${common_1.convertDateToISOString(minAgeDate)}' `;
            ...
            if (common_1.convertStringToBoolean(include_unspecified_move_in_date)) {
           ...
           OR (move_in_date_from IS NULL AND move_in_date_to IS NULL)) `;
            ...
            if (subscribe_from) {
                filterQuery = filterQuery + `AND application.created_at >= '${common_1.convertDateToISOString(new Date(subscribe_from))}' `;
            }
            if (subscribe_to) {
                filterQuery = filterQuery + `AND application.created_at < '${common_1.convertDateToISOString(new Date(subscribe_to))}' `;
            }
            if (email) {
            ...
            let dbQuery = typeorm_1.getRepository(entity_1.Application, process.env.NODE_ENV)
                .createQueryBuilder('application')
                .innerJoin(entity_1.User, 'user', 'user.iduser = application.iduser')
                .innerJoin(entity_1.UserProfiles, 'userProfiles', 'userProfiles.iduser = application.iduser')
                .innerJoin(entity_1.UserPreferences, 'userPreferences', 'user.user_key = userPreferences.iduser')
                .leftJoin(entity_1.Offer, 'offer', 'offer.iduser = application.iduser')
                .leftJoin(entity_1.Contract, 'contract', 'contract.iduser = application.iduser')
                .leftJoinAndSelect(subquery => subquery
                .from(entity_1.User, 'user')
                .innerJoin(entity_1.UserPreferredRoommates, 'userPreferredRoommates', 'userPreferredRoommates.inviter_id = user.iduser OR userPreferredRoommates.invitee_id = user.iduser')
                .innerJoin(entity_1.UserPreferences, 'userPreferences', 'userPreferences.iduser = user.user_key')
                .select(preferredRoommatesFields), 'user_preferred_roommates', '(user_preferred_roommates.inviter_id = user.iduser OR user_preferred_roommates.invitee_id = user.iduser) AND user_preferred_roommates.iduser != user.iduser')
                .leftJoin(entity_1.AdminMemberNotes, 'adminMemberNotes', 'adminMemberNotes.iduser = user.iduser')
        ...
        this.getTotalNumberOfUsersByClientId = () => typeorm_1.getRepository(entity_1.User, process.env.NODE_ENV)
            .createQueryBuilder(this.REPO_NAME)
            ...
            .getRawOne();
        this.getCountOfUsersByDateRange = (startDate, endDate) => typeorm_1.getRepository(entity_1.User, process.env.NODE_ENV)
            .createQueryBuilder(this.REPO_NAME)
